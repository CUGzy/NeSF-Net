...
class FMS_block(nn.Module):
    def __init__(self, in_ch, out_ch,num_heads=8, window_size=4):
        super(FMS_block, self).__init__()
        self.glb = GlBlock(dim=in_ch,outdim=in_ch,num_heads=num_heads, window_size=window_size)
        self.localb=multilocalBlock(dim=in_ch,outdim=in_ch,num_heads=8, window_size=window_size)

        self.outconv_bn_relu_glb = nn.Sequential(
            nn.Conv2d(in_ch+in_ch, out_ch, kernel_size=1, stride=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
        )
        self.outconv_bn_relu_local = nn.Sequential(
    ...
    def forward(self, x):
        glb = self.glb(x)
        local = self.localb(x)
        fuse = torch.cat([glb,local],dim=1)
        fuse = self.outconv_bn_relu_glb(fuse)
